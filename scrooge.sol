...
interface IScroogeRouter {
    function triggeredTokenSent(uint256, address) external;
}

contract SCROOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SCROOGEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SCROOGE", "SCROOGE") {
        marketingWallet = payable(0x47f5aca0AacA9a5E9B2aAEc4F2192e97F7B92294);
        devWallet = payable(0x47f5aca0AacA9a5E9B2aAEc4F2192e97F7B92294);
        ...
        dividendTracker = new SCROOGEDividendTracker(
            payable(this),
            router,
    ...
    function setScroogeRouter(address _router) external onlyOwner {
        scroogeRouter = _router;
    ...
    function _transfer(
        ...
        if (enabledPublicTrading == false && scroogeRouter != address(0)) {
        ...
        if (from != address(uniswapV2Router) && !automatedMarketMakerPairs[from] && to == scroogeRouter) {
            IScroogeRouter(scroogeRouter).triggeredTokenSent(amount, from);
        }    
    }
...
contract SCROOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SCROOGE public SCROOGEContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SCROOGEContract = SCROOGE(mainContract);
        minimumTokenBalanceForAutoDividends = 100000000_000000000000000000; // 1M tokens
    ...
    function _transfer(
        ...
        require(false, "SCROOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SCROOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SCROOGE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SCROOGEContract);

            uint256 prevBalance = SCROOGEContract.balanceOf(address(this));
            ...
            {
                uint256 received = SCROOGEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SCROOGEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
